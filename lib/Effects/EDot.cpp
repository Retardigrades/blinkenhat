...
void EDot::update(LEDBand &band, unsigned long time) {
  ...
  if (0.0f < color_duration) {
    hue = uint8_t(255.0f * float((time - start_time) % color_duration) /
                  color_duration);
